...
namespace ExpanseMod.LootSpawn
{
    ...
    {
        private const string _zonePrefix = "Conflict Zone";
        private static TimeSpan _timeToLive = new TimeSpan(0, 0, 10); //Default of 10 minutes
        private ZoneItemReward _reward { get; set; }

        public ConflictZone(Vector3D position, double radius) 
            : base(_zonePrefix, position, radius, _timeToLive, true)
        {
            //TODO: Make the reward configurable
            _reward = new ZoneItemReward()
            {
                ItemCount = 10,
                ItemDefinition = new MyDefinitionId(typeof(MyObjectBuilder_Ore),"Stone")
            };
        }
        ...
        public override ZoneUpdateResult Update(List<IMyPlayer> players)
        {
            try
            {
                Logger.Display("Checking if Conflict Zone should expire...");
                //Check if we need to expire before thinking
                if (base.Update(players) == ZoneUpdateResult.Timeout)
                {
                    Logger.Display("Checking for winners...");
